...
class AuthService:
    """
    Orchestrates authentication, lock-out, and token creation.
    """
    ...
    def check_account_locked(self, email: str) -> tuple[bool, int | None]:
        """
        Returns (is_locked, remaining_seconds or None)
        """
    ...
    def increment_failed_attempts(self, email: str) -> None:
        ...
        max_attempts    = settings.ACCOUNT_LOCKOUT.get("MAX_FAILED_ATTEMPTS", 5)
        lockout_seconds = settings.ACCOUNT_LOCKOUT.get("LOCKOUT_DURATION", 15 * 60)

        if lockout_data["attempts"] >= max_attempts:
            lockout_data["locked_until"] = timezone.now().timestamp() + lockout_seconds
            cache.set(cache_key, lockout_data, lockout_seconds)
        else:
            cache.set(cache_key, lockout_data, 24 * 60 * 60)  # keep for 24 h

    def reset_failed_attempts(self, email: str) -> None:
        cache.delete(self._get_lockout_cache_key(email))

    def login(self, *, email: str, password: str) -> dict | None:

        # 1) Lock-out check
        is_locked, secs_left = self.check_account_locked(email)
        if is_locked:
            mins, secs = divmod(secs_left, 60)
            wait_str   = f"{mins} minutes and {secs} seconds" if mins else f"{secs} seconds"
            return {
                "locked": True,
                "message": f"Account is locked due to too many failed attempts. "
        ...
        # 3) Verify e-mail activation
        print(user.is_active)
        if not user.is_active:
            return {
                "inactive": True,
                "message": "Account not verified. Please check your e-mail inbox."
            }
