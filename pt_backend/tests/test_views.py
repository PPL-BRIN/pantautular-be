...
class TopLocalPortalsViewTest(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.api_key = os.getenv("SECRET_API_KEY", "test-api-key")
        self.client.credentials(HTTP_X_API_KEY=self.api_key)
    ...
    def test_get_top_portals_success(self):
        ...
        self.mock_service_instance.get_top_local_portals.return_value = mock_data

        # Make request
        response = self.client.get('/api/dashboard/top-local-portals/')

        # Assertions
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["local_top"], mock_data)
        self.mock_service_instance.get_top_local_portals.assert_called_once()

    def test_get_top_portals_empty_result(self):
        # Mock empty result
        self.mock_service_instance.get_top_local_portals.return_value = []
    ...
    def test_get_top_portals_repository_error(self):
        # Mock service error
        self.mock_service_instance.get_top_local_portals.side_effect = Exception("Database error")
    ...
    def test_get_top_portals_error_response_from_repository(self):
        ...
        error_response = {"error": "Error retrieving local portals"}
        self.mock_service_instance.get_top_local_portals.return_value = error_response
    ...
    def test_get_top_portals_queryset_conversion(self):
        ...
        self.assertTrue(isinstance(response.json()["local_top"], list))
    ...
    def test_news_service_exception_propagation(self):
        ...
        try:
            ...
            mock_repository.get_top_five_local_portals.side_effect = Exception("Repository error")

            # Test if the service correctly re-raises the exception
            with self.assertRaises(Exception):
                service = NewsService(mock_repository)
                service.get_top_local_portals()
        finally:
...
class LocalPortalStatisticsViewTest(TestCase):
    ...
    def test_get_portal_statistics_success(self):
        ...
        self.mock_service_instance.get_local_portal_statistics.return_value = mock_data
        # Make request
        response = self.client.get('/api/dashboard/local-portal-stats/')

        # Assertions
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()['local_stats'], mock_data)
        self.mock_service_instance.get_local_portal_statistics.assert_called_once()

    def test_get_portal_statistics_empty_result(self):
        # Mock empty result
        self.mock_service_instance.get_local_portal_statistics.return_value = []
    ...
    def test_get_portal_statistics_repository_error(self):
        # Mock service error
        self.mock_service_instance.get_local_portal_statistics.side_effect = Exception("Database error")
    ...
    def test_get_portal_statistics_error_response_from_repository(self):
        # Mock repository returning an error dictionary
        error_response = {"error": "Error retrieving local portal statistics"}
        self.mock_service_instance.get_local_portal_statistics.return_value = error_response
    ...
    def test_get_portal_statistics_queryset_conversion(self):
        ...
        self.assertEqual(len(response.json()['local_stats']), 2)
    ...
    def test_service_propagates_exceptions(self):
        ...
        try:
            ...
            mock_repository = Mock()
            mock_repository.get_local_portal_statistics.side_effect = Exception("Repository error")

            # Test if the service correctly re-raises the exception
            with self.assertRaises(Exception):
                service = NewsService(mock_repository)
                service.get_local_portal_statistics()
        finally:
            # Restart the patcher for other tests
            self.mock_news_service = self.patcher.start()
            self.mock_service_instance = Mock()
