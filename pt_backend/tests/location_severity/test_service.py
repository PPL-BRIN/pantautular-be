...
class LocationServiceTestCase(TestCase):
    ...
    def test_get_province_severity_stats(self):
        ...
        self.mock_repository.get_location_severity_stats.return_value = expected_result
        
        # Call the service method (no parameter needed now)
        result = self.service.get_location_severity_stats()
        
        # Assert repository method was called without parameters
        self.mock_repository.get_location_severity_stats.assert_called_once_with()
        
        # Assert service returns repository result
        self.assertEqual(result, expected_result)
        
    def test_get_location_severity_stats_with_default_repository(self):
        ...
        with patch('pt_backend.services.LocationRepository') as mock_repo_class:
            ...
            mock_repo_instance.get_location_severity_stats.return_value = expected_result
            
            # Create service with default repository
            service = LocationService()
            result = service.get_location_severity_stats()
            
            # Assert default repository was used
            mock_repo_class.assert_called_once()
            mock_repo_instance.get_location_severity_stats.assert_called_once_with()
